...
export class Taborok {
    ...
    constructor() {
        const data: string = fs.readFileSync('taborok.txt', 'utf8');
    ...
    private sorszam(ho: number, nap: number): number {
        if (ho === 6) {
            return nap - 15;
        } else if (ho === 7) {
            return nap + 15;
    ...
    public getTartTaborokSzama(h: number, n: number): number {
        ...
        return this.taborok.filter(
            tabor =>
                this.sorszam(tabor.kezdoHo, tabor.kezdoNap) <= napSorszam &&
                napSorszam <= this.sorszam(tabor.vegHo, tabor.vegNap)
        ).length;
    }
}